...
V2_API_KEY = control.addon('plugin.video.blacklodge').getSetting('trakt.client_id')
CLIENT_SECRET = control.addon('plugin.video.blacklodge').getSetting('trakt.client_secret')
...
def getTraktCredentialsInfo():
    user = control.addon('plugin.video.blacklodge').getSetting('trakt.user').strip()
    token = control.addon('plugin.video.blacklodge').getSetting('trakt.token')
    refresh = control.addon('plugin.video.blacklodge').getSetting('trakt.refresh')
    if (user == '' or token == '' or refresh == ''): return False
    return True


def getTraktIndicatorsInfo():
    indicators = control.addon('plugin.video.blacklodge').getSetting('indicators') if getTraktCredentialsInfo() == False else control.addon('plugin.video.blacklodge').getSetting('indicators.alt')
...
def cachesyncMovies(timeout=0):
    indicators = cache.get(syncMovies, timeout, control.addon('plugin.video.blacklodge').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncMovies():
    timeout = cache.timeout(syncMovies, control.addon('plugin.video.blacklodge').getSetting('trakt.user').strip())
    return timeout
...
def cachesyncTVShows(timeout=0):
    indicators = cache.get(syncTVShows, timeout, control.addon('plugin.video.blacklodge').getSetting('trakt.user').strip())
    return indicators


def timeoutsyncTVShows():
    timeout = cache.timeout(syncTVShows, control.addon('plugin.video.blacklodge').getSetting('trakt.user').strip())
