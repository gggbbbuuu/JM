...
def get_ua():
    try:
        last_gen = int(control.setting('last_ua_create'))
    except:
        last_gen = 0
    if not control.setting('current_ua') or last_gen < (time.time() - (7 * 24 * 60 * 60)):
        ...
        control.setSetting('current_ua', user_agent)
        control.setSetting('last_ua_create', str(int(time.time())))
    else:
        user_agent = control.setting('current_ua')
    return user_agent
