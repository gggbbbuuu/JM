...
class AbstractUriCrawler(AbstractCrawler):
    ...
    def _get_m3u8_obj_with_best_bandwitdth(self):
        ...
        if m3u8_obj.is_variant:
            ...
            import xbmcgui
            # xbmcgui.Dialog().textviewer('m3u8_obj', str(m3u8_obj.__dict__))
            resolutions = [str(x.stream_info.resolution)+' | '+str(x.stream_info.bandwidth) for x in m3u8_obj.playlists]
            req = xbmcgui.Dialog().select('Select Resolution', resolutions)
            if req == -1:
                raise Exception("No stream selected")
            for playlist in m3u8_obj.playlists:
                if str(playlist.stream_info.resolution)+' | '+str(playlist.stream_info.bandwidth) == resolutions[req]:
