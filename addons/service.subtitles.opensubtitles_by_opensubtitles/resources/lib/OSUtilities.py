...
ua = base64.b64decode('49mYltWdqVWa29Wb'[::-1]).decode('utf-8')
...
class OSDBServer:
    def __init__(self, *args, **kwargs):
        ...
        login = self.server.LogIn(__addon__.getSetting("OSuser"), __addon__.getSetting("OSpass"), 'en', ua)
        # login = self.server.LogIn(__addon__.getSetting("OSuser"), __addon__.getSetting("OSpass"), "en", "{0}_v{1}".format(__scriptname__.replace(" ", "_"), __version__))
        ...
        if (self.osdb_token):
            ...
            if item['mansearch']:
                searchlist = [{'sublanguageid': ",".join(item['3let_language']),
                               'query': urllib_parse.unquote(item['mansearchstr'])
                               }]
...
def log(module, msg):
    xbmc.log(u"### [{0}] - {1}".format(module, msg), level=xbmc.LOGDEBUG)
...
def OpensubtitlesHashRar(firsrarfile):
    ...
    for i in range(4):
        f.seek(max(0, seek), 0)
        a = f.read(100)
        typ, flag, size = struct.unpack('<BHH', a[2:2 + 5])
        if 0x74 == typ:
