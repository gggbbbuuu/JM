	...
	def __init__(self):
		for id in properties:
			if xbmcaddon.Addon().getSetting(id) == 'true':
				...
				xbmc.log('[ context.trailer.mod ]  Context menu item enabled: {0}'.format(id), LOGNOTICE)
	...
	def onSettingsChanged(self):
		for id in properties:
			if xbmcaddon.Addon().getSetting(id) == 'true':
				...
				xbmc.log('[ context.trailer.mod ]  Context menu item disabled: {0}'.format(id), LOGNOTICE)

# start monitoring settings changes events
xbmc.log('[ context.trailer.mod ]  Service Started', LOGNOTICE)
properties_monitor = PropertiesUpdater()

# wait until abort is requested
properties_monitor.waitForAbort()
xbmc.log('[ context.trailer.mod ]  Service Stopped', LOGNOTICE)