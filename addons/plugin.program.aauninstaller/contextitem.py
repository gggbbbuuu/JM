...
def removeaddon(id=''):
    ...
    addonsinfo.sort(key=lambda x: x[0].lower())
    addonslistrange = list(range(len(addonsinfo)))
    selected = []
    idindex = next(i for i, v in enumerate(addonsinfo) if id in v)
    selected.append(idindex)
...
if __name__ == '__main__':
    ...
    info = xbmc.getInfoLabel('ListItem.Property(Addon.ID)')
    removeaddon(info)
