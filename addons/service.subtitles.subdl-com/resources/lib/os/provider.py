...
class SubtitlesProvider:
    ...
    def search_subtitles(self, media_data: dict):
        pref_langs = get_language_data(self.params).get("languages").upper()
        metadata = self.parse_filename(media_data['query'])
        tmdbID = self.get_tmdb_id(metadata)
        url = f"{API_URL}?api_key={self.api_key}&type={metadata['type']}&languages={pref_langs}&tmdb_id={tmdbID}"
        if metadata['type'] == 'tv':
        ...
        subslist = sorted(result["subtitles"], key=lambda d: d['lang']) if result["subtitles"] else None
        return subslist
