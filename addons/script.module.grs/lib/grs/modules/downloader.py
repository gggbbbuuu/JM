...
def doDownload(url, dest, title, image, headers):
    ...
    if not resp:
        xbmcgui.Dialog().ok('Stream File Download', dest + '[CR]' + 'Download failed' + '[CR]' + 'No response from server')
        return
    ...
    if content < 1:
        xbmcgui.Dialog().ok('Stream File Download', file + '[CR]' + 'Unknown filesize' + '[CR]' + 'Unable to download')
        return
    ...
    if xbmcgui.Dialog().yesno('Stream File Download' + ' - Confirm Download', file + '[CR]' + 'Complete file is %dMB' % mb + '[CR]' + 'Continue with download?', 'Confirm',  'Cancel'):
        return
