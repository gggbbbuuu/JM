...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(resource.images.blacklodge.artwork)'):
        return os.path.join(xbmcaddon.Addon('resource.images.blacklodge.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(resource.images.blacklodge.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://resource.images.blacklodge.artwork)')
