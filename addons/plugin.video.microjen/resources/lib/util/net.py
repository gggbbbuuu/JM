...
ADDON = xbmcaddon.Addon('plugin.video.microjen')

def get_ua():
    try:
        last_gen = int(ADDON.get_setting('last_ua_create'))
    except:
        last_gen = 0
    if not ADDON.get_setting('current_ua') or last_gen < (time.time() - (7 * 24 * 60 * 60)):
